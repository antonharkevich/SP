...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_DRAWITEM:
        ...
        if (lpInfo->CtlID == ID_BUTTON_1) {
            ...
            FillRect(lpInfo->hDC, &rcItem, CreateSolidBrush(RGB(255, 165, 0)));
            POINT arr[4];
            INT width, height;
            ...
            arr[2] = { width / 2 + width / 4, height / 2 - height / 4 };
            Polyline(lpInfo->hDC, arr, 4);
        }
        if (lpInfo->CtlID == ID_BUTTON_2) {
            RECT rcItem;
            rcItem = lpInfo->rcItem;
            FillRect(lpInfo->hDC, &rcItem, CreateSolidBrush(RGB(0, 255, 0)));
            POINT arr[4];
            INT width, height;
            width = rcItem.right - rcItem.left;
            height = rcItem.bottom - rcItem.top;
